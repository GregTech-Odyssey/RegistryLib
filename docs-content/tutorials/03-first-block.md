...
3. **`.initialProperties(() -> Blocks.STONE)`** copies properties from an existing block, such as hardness and sound.
4. **`.lang("Decorative Stone")`** sets the English display name for datagen.
5. **`.simpleItem()`** generates a default `BlockItem` so the block can appear in inventories and creative tabs.
6. **`.register()`** submits the registration and returns a `BlockEntry<Block>`.
...
4. **`.lang(...)`** and **`.lang(ModRegistryCore.LANG_ZH_CN, ...)`** set English and another locale.
...
- **`.item(item -> ...)`** creates a BlockItem and lets you configure it with tooltips, tabs, tags, and other item builder options.
- **No item call** means the block exists in the world but has no inventory representation.
:::
...
4. Place the block and try mining it with the correct tool tier for Magic Ore.
5. Confirm the loot drops match your configuration.

## Related

- [Register Blocks (How-To)](/how-to/register-blocks) gives the full reference for all block builder options
- [Understanding the Registration Chain](/tutorials/understanding-chain) explains how the fluent API works under the hood
