...
public class RegistryLib {
  ...
  private final Table<ResourceKey<? extends Registry<?>>, String, Registration<?, ?>>
      registrations = HashBasedTable.create();
  private final Multimap<Pair<String, ResourceKey<? extends Registry<?>>>, Consumer<?>>
  ...
  private final Table<
      ...
      datagensByEntry = HashBasedTable.create();
  private final ListMultimap<GeneratorType<?>, Consumer<?>> datagens = ArrayListMultimap.create();
  private final Multimap<ResourceKey<CreativeModeTab>, Consumer<CreativeModeTabModifier>>
      creativeModeTabModifiers = ArrayListMultimap.create();
  ...
  @Nullable private IEventBus modEventBus;
  private boolean skipErrors;

  // === Constructor + Factory ===

  protected RegistryLib(String modid) {
    this.modid = modid;
  }

  public static RegistryLib create(String modid) {
    var ret = new RegistryLib(modid);
    Optional<IEventBus> modEventBus =
        ModList.get().getModContainerById(modid).map(ModContainer::getEventBus);
    modEventBus.ifPresentOrElse(
        ret::registerEventListeners,
        () -> {
          String message =
              "# [RegistryLib] Failed to register eventListeners for mod " + modid + " #";
  ...
  public RegistryLib registerEventListeners(IEventBus bus) {
    ...
    if (registrationsForType.size() > 0) {
      ...
      for (Entry<String, Registration<?, ?>> e : registrationsForType.entrySet()) {
        try {
          ...
          String err =
              "Unexpected error while registering entry "
                  + e.getValue().getName()
                  + " to registry "
                  + event.getRegistryKey().identifier();
          if (skipErrors) {
  ...
  protected void onBuildCreativeModeTabContents(BuildCreativeModeTabContentsEvent event) {
    ...
    creativeModeTabModifiers.forEach(
        (key, value) -> {
          if (event.getTabKey().equals(key)) value.accept(modifier);
        });
  }

  @Nullable private RegistryLibDataProvider provider;
  ...
  public <R, T extends R> Collection<RegistryEntry<R, T>> getAll(
      ResourceKey<? extends Registry<R>> type) {
    return registrations.row(type).values().stream()
  ...
  @SuppressWarnings("unchecked")
  public <R, T extends R> RegistryLib addRegisterCallback(
    ...
    if (reg == null) {
      registerCallbacks.put(Pair.of(name, registryType), (Consumer<?>) callback);
    } else {
  ...
  public <R> RegistryLib addRegisterCallback(
  ...
  public <P, R> RegistryLib setDataGenerator(
  ...
  public <T> RegistryLib addDataGenerator(
  ...
  @Nullable private DataProviderInitializer initializer;
  ...
  private final Supplier<List<Pair<String, String>>> extraLang =
      Lazy.of(
          () -> {
  ...
  private Optional<Pair<String, ResourceKey<? extends Registry<?>>>> getEntryForGenerator(
    ...
    for (Map.Entry<Pair<String, ResourceKey<? extends Registry<?>>>, Consumer<?>> e :
        datagensByEntry.column(type).entrySet()) {
  ...
  public RegistryLib skipErrors(boolean skipErrors) {
  ...
  public RegistryLib defaultCreativeTab(ResourceKey<CreativeModeTab> creativeModeTab) {
    defaultCreativeModeTab = creativeModeTab;
    return this;
  }

  public RegistryLib modifyCreativeModeTab(
  ...
  public <R> ResourceKey<Registry<R>> makeRegistry(
    ...
    final ResourceKey<Registry<R>> registryId =
  ...
  @StandardAPI
  public <R, T extends R> NoConfigBuilder<R, T, RegistryLib> generic(
      @Nonnull String name,
      @Nonnull ResourceKey<Registry<R>> registryType,
      @Nonnull Supplier<T> factory) {
    return entry(
  ...
  public <T extends Item> ItemEntry<T> item(
      ...
      @Nonnull Consumer<ItemBuilder<T, RegistryLib>> config) {
  ...
  public <T extends Item, P> ItemBuilder<T, P> item(
    ...
    return entry(
        callback ->
            ItemBuilder.create(this, parent, name, callback, factory)
                .transform(
                    builder ->
  ...
  public <T extends Block> BlockEntry<T> block(
      ...
      @Nonnull Consumer<BlockBuilder<T, RegistryLib>> config) {
  ...
  public <T extends BlockEntity> BlockEntityEntry<T> blockEntity(
      ...
      @Nonnull Consumer<BlockEntityBuilder<T, RegistryLib>> config) {
  ...
  @StandardAPI(
      "Consumer-scoped: configures and auto-registers a Fluid entry with default Flowing type.")
  public FluidEntry<BaseFlowingFluid.Flowing> fluid(
      @Nonnull String name,
      @Nonnull Identifier stillTexture,
      @Nonnull Identifier flowingTexture,
      @Nonnull Consumer<FluidBuilder<BaseFlowingFluid.Flowing, RegistryLib>> config) {
  ...
  @StandardAPI(
      "Consumer-scoped: configures and auto-registers a Fluid entry with custom FluidFactory.")
  public <T extends BaseFlowingFluid> FluidEntry<T> fluid(
      @Nonnull String name,
      @Nonnull Identifier stillTexture,
      @Nonnull Identifier flowingTexture,
      @Nonnull FluidBuilder.FluidFactory<T> fluidFactory,
      @Nonnull Consumer<FluidBuilder<T, RegistryLib>> config) {
  ...
  public <T extends BaseFlowingFluid, P> FluidBuilder<T, P> fluid(
    ...
    return entry(
            callback ->
                FluidBuilder.create(this, parent, name, callback, FluidType::new, fluidFactory))
  ...
  @StandardAPI
  public NoConfigBuilder<CreativeModeTab, CreativeModeTab, RegistryLib> defaultCreativeTab(
      String name) {
