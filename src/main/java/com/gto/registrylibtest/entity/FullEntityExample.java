 ...
 *
 * <p>与 {@link SimpleEntityExample} 的区别：
 *
 * <ul>
 *   <li>使用传统 <b>GoalSelector</b> AI 而非 Brain AI</li>
 *   <li>使用 {@link SynchedEntityData} 进行服务端→客户端实时数据同步</li>
 *   <li>展示所有 EntityBuilder 方法（fireImmune、properties、spawnEgg 自定义等）</li>
 * </ul>
 *
...
public class FullEntityExample {
     ...
     *
     * <ul>
     *   <li><b>GoalSelector</b> — 经典的基于优先级的 AI 系统</li>
     *   <li><b>SynchedEntityData</b> — 自动将 {@code IS_ENRAGED} 从服务端同步到客户端</li>
     *   <li><b>hurtServer</b> — 受伤时根据血量百分比切换狂暴状态</li>
     * </ul>
     *
     * <h3>GoalSelector vs Brain</h3>
     *
     * <p>GoalSelector 基于优先级队列，每 tick 选择最高优先级可运行的 Goal。适合中低复杂度 AI。
     * Brain 系统使用 Sensor → Memory → Behavior 分层，适合复杂行为编排（见 {@link CrystalGuardianAi}）。
     */
    public static class ObsidianGolem extends PathfinderMob {
        ...
        private static final EntityDataAccessor<Boolean> IS_ENRAGED =
                SynchedEntityData.defineId(ObsidianGolem.class, EntityDataSerializers.BOOLEAN);
        ...
        /**
         * 注册属性 — 必须通过 EntityBuilder.attributes() 或 EntityAttributeCreationEvent 注册，
         * 否则实体创建时将导致 NullPointerException。
         */
        ...
        protected void registerGoals() {
            ...
            targetSelector.addGoal(2, new NearestAttackableTargetGoal<>(               // 主动寻找最近玩家
     ...
     *
     * <p>使用 {@link EntityRenderer} + {@link BlockStateModel} 实现方块模型复用，
     * 无需创建自定义模型文件。适合方块形态的简单实体。
     *
